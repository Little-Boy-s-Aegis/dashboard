    ...
    <div style={{ animation: 'fadeInUp 0.25s ease-out', display: 'flex', flexDirection: 'column', gap: 16 }}>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: '1.2fr 1fr', gap: 16 }}>
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: 16 }}>
          ...
          {/* Live Audit Summary */}
          <div className="glass-panel" style={{ padding: 16 }}>
            <h3 style={{ fontSize: '0.85rem', color: 'var(--text-0)', marginBottom: 12 }}>SOAR Audit Summary</h3>
            <div style={{ display: 'flex', flexDirection: 'column', gap: 8 }}>
              {[
                {
                  label: 'Engine Status',
                  value: autopilotEnabled ? 'Autopilot Active' : 'Suggest-Only Mode',
                  color: autopilotEnabled ? 'var(--low)' : 'var(--accent)',
                  icon: <Zap size={12} />
                },
                {
                  label: 'Total Actions Executed',
                  value: `${metrics.totalPlaybooks} actions (${metrics.successCount} success)`,
                  color: 'var(--info)',
                  icon: <Play size={12} />
                },
                {
                  label: 'Active Firewall Bans',
                  value: `${activeBanTotal} IP${activeBanTotal !== 1 ? 's' : ''} blocked at WAF edge`,
                  color: activeBanTotal > 0 ? 'var(--critical)' : 'var(--text-3)',
                  icon: <ShieldCheck size={12} />
                },
                {
                  label: 'Last Containment Action',
                  value: soarActions.length > 0
                    ? new Date(soarActions[0].timestamp).toLocaleString('en-GB', { day: '2-digit', month: 'short', year: 'numeric', hour: '2-digit', minute: '2-digit', second: '2-digit', hour12: false })
                    : 'No actions recorded',
                  color: 'var(--text-1)',
                  icon: <Clock size={12} />
                },
              ].map((item, i) => (
                <div key={i} style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', padding: '7px 10px', background: 'rgba(0,0,0,0.15)', border: '1px solid var(--border-0)', borderRadius: 'var(--r-xs)' }}>
                  <span style={{ display: 'flex', alignItems: 'center', gap: 6, fontSize: '0.72rem', color: 'var(--text-3)' }}>
                    {item.icon}
                    {item.label}
                  </span>
                  <span style={{ fontSize: '0.72rem', fontWeight: 600, fontFamily: "'IBM Plex Mono', monospace", color: item.color }}>
                    {item.value}
                  </span>
                </div>
              ))}
