...
export default function FimDashboard({ fimEvents, onRefresh }: Props) {
  ...
  const getDiff = (p: string) => {
    ...
    if (p.endsWith('.locked')) return { f: p, d: ['+ -----BEGIN AES ENCRYPTED DATA-----', '+ K8s9Xw1mP02sL9mK1s0Dns9Kl2sLd89Km2sLns93ksLkd9sK2', '+ -----END AES ENCRYPTED DATA-----'] };
    return { f: p, d: ['  [Binary file — unable to diff]', '  Verify checksums below.'] };
  };
